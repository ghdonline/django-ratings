...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for vote in orm.Vote.objects.all():
            try:
                model_class = ContentType.objects.get(pk=vote.content_type.pk).model_class()
                if model_class:
                    instance = model_class.objects.get(pk=vote.object_id)
                    vote.top_ancestor_id = instance.top_ancestor.id
                    vote.save()
            except AttributeError:
                pass

        for score in orm.Score.objects.all():
            try:
                model_class = ContentType.objects.get(pk=score.content_type.pk).model_class()
                if model_class:
                    instance = model_class.objects.get(pk=score.object_id)
                    score.top_ancestor_id = instance.top_ancestor.id
                    score.save()
            except AttributeError:
                pass
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 4, 18, 22, 43, 2, 942682)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 4, 18, 22, 43, 2, 942646)'}),
        ...
        'djangoratings.vote': {
            ...
            'date_added': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 4, 18, 22, 43, 2, 941931)'}),
            'date_changed': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 4, 18, 22, 43, 2, 941976)'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
