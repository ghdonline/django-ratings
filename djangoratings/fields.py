...
class RatingManager(object):
    ...
    def add(self, score, user, ip_address, cookies={}, commit=True):
        ...
        # Store the ID of the top ancestor (e.g. community or panel) for
        # faster filtering of votes. We use this for visibility filtering,
        # since some content lives in private communities. This filter
        # makes it easier to run the query "Show me the last 20
        # recommended pieces of content in the communities this user can see."
        top_ancestor_id = None
        try:
            top_ancestor_id = self.instance.top_ancestor.id
        except AttributeError:
            # No top_ancestor -- probably a non-Node model. Ignore.
            pass

        kwargs = dict(
            content_type    = self.get_content_type(),
            object_id       = self.instance.pk,
            top_ancestor_id = top_ancestor_id,
            key             = self.field.key,
            user            = user,
        )
        if not user:
